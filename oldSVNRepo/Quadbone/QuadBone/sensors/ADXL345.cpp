...
#define ADXL345_TO_READ (6)      // num of chars we are going to read each time (two chars for each axis)
namespace std {
ADXL345::ADXL345() {
	status = ADXL345_OK;
...
void ADXL345::writeTo(char address, char val) {
	...
	i2c->writei2c(address, val);

}

// Reads num chars starting from address register on device in to _buff array
void ADXL345::readFrom(char address, int num, char _buff[]) {

	int i = 0;
	for (i = 0; i < num; i++) {
		_buff[i] = i2c->readi2c(address);
	}
	if (i != num) {
...
void ADXL345::getRangeSetting(char* rangeSetting) {
	char _b;
...
void ADXL345::set_bw(char bw_code) {
...
int ADXL345::constrain(int value, int constrain_min, int constrain_max) {
	if (value < constrain_min)
		return constrain_min;
	else if (value > constrain_max)
