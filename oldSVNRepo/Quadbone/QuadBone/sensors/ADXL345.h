...
private:
	...
	void readFrom(char address, int num, char buff[]);
	void setRegisterBit(char regAdress, int bitPos, bool state);
	bool getRegisterBit(char regAdress, int bitPos);
	int constrain(int value, int constrain_min, int constrain_max);
	char _buff[6]; //6 chars buffer for saving data read from the device
	I2C *i2c;
};
}
#endif
